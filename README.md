...
### v27-v28 · 10 juillet 2026 · Correction du pont widgets
- Pont de données widgets réparé : mauvais nom de plugin (`CapacitorWidgetBridge` au lieu de `WidgetBridgePlugin`) et méthode inexistante (`sendWidgetData`), erreurs jusque-là masquées par un `catch` vide
- Ajout de `setRegisteredWidgets()` (une fois par session) puis de `reloadAllTimelines()` après l'écriture des données
- Ajout des clés `week_fixed`, `week_sliding` et `charge_weeks` au payload : absentes depuis l'introduction des widgets semaine/charge, ce qui les laissait vides

### v33-v35 · 11 juillet 2026 · Refonte des 7 widgets
- Taille par défaut standardisée à 3×1 pour les sept widgets (Prochain événement, Agenda du jour, À rendre, Progression, Semaine fixe, Semaine glissante, Charge par semaine)
- Le widget « Cette semaine » affiche 7 jours au lieu de 5
- Code matière visible sur tous les widgets, y compris les ateliers (résolu via l'attribution de matière `getGroupTag(uid)?.subject`, le champ code n'existant pas directement sur les événements de groupe)
- Widgets « Prochain événement » et « Agenda du jour » repensés : icônes par type d'événement, badge d'urgence coloré, bouton « Rejoindre » séparé
- Correctifs : nombre de lignes fixe sur Progression/Charge (l'ancien calcul adaptatif était trop conservateur et gaspillait l'espace), suppression d'un point d'indicateur en double sur Agenda du jour, colonne horaire élargie pour éviter le retour à la ligne sur les widgets semaine, exclusion des matières archivées du widget Progression
- Payload widgets en v3 : ajout des champs code, intervenant, lien de réunion et statut « en cours »
- Sept nouvelles ressources drawable pour les icônes de type d'événement

### Hérité du dépôt web (fichier `index.html` partagé)
- Archivage des matières terminées, tableau « Moyennes par matière », correctif de semaine ISO. Détail dans le README du dépôt web (`E:\GIT\Brightspace_agenda`)

> ⚠️ Connu : `WidgetUpdateReceiver` et l'action `com.brightspaceagenda.WIDGET_UPDATE` restent orphelins, rien ne les déclenche encore
